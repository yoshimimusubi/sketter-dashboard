...
REQUIRED_COLUMNS = [
"法人名", "取引先名", "加藤さん記載欄", "●A応募件数30回以上", "●B応募件数30−15回",
"●C応募件数14−1回", "●D応募件数0回", "アンケート結果", "●◎回収できている。かつトライアル後も継続。",
"●△トライアル後で終了したい。", "●✖️回収していない。", "フェーズ", "メールアドレス",
"契約確度", "最終連絡日", "フォロー状況", "ご意向", "現状", "電話番号", "担当者",
"サブメールアドレス", "契約に向けてのチェック項目"
]
...
# 欠損カラムのチェック（完全一致でなくても動くように警告のみに変更）
missing = [c for c in REQUIRED_COLUMNS if c not in df.columns]
if missing:
st.warning(f"【注意】一部の推奨カラムが見つかりません: {', '.join(missing)}")
...
achieved_amount = int(achieved_df["想定金額"].sum())
...
fig = go.Figure()
fig.add_trace(go.Bar(y=["金額ベース"], x=[achieved_amount], name="確定済", marker_color="#0047AB", orientation='h'))
fig.add_trace(go.Bar(y=["金額ベース"], x=[amount_A], name="確度A (80%)", marker_color="#007BBB", orientation='h'))
fig.add_trace(go.Bar(y=["金額ベース"], x=[amount_B], name="確度B (50%)", marker_color="#4FA8D1", orientation='h'))
fig.add_trace(go.Bar(y=["金額ベース"], x=[amount_C], name="確度C (20%)", marker_color="#93C5FD", orientation='h'))
fig.add_trace(go.Bar(y=["金額ベース"], x=[gap_amount], name="不足分 (ギャップ)", marker_color="#E2E8F0", orientation='h'))
fig.update_layout(barmode='stack', height=200, margin=dict(t=30, b=30, l=10, r=10), paper_bgcolor="rgba(0,0,0,0)",
plot_bgcolor="rgba(0,0,0,0)")
st.plotly_chart(fig, use_container_width=True)


# ---------------------------------------------------------------------------
# 【新設】今週の行動計画 (Weekly Action Plan)
# ---------------------------------------------------------------------------
...
pipeline_data += f"- {name} / ご意向: {row.get('ご意向','')} / 現状: {row.get('現状','')} / フォロー状況: {row.get('フォロー状況','')} /
チェック項目: {row.get('契約に向けてのチェック項目','')}\n"
...
2. Bヨミ、Cヨミを次のフェーズに引き上げるためのアプローチ（アンケート結果やチェック項目をフックにした提案など）
...
alert_mask = (df["最終連絡日"].notna() & ((today - df["最終連絡日"]).dt.days >= 7) & ~df["フェーズ"].astype(str).apply(is_completed))
...
st.markdown(f'<div class="alert-banner">【重要】最終連絡から7日以上経過している案件が {len(alert_df)} 件あります。早急なフォローをご検討ください。</div>',
unsafe_allow_html=True)
